...
> ⚠️ _#1514 note (My Documents wallet):_ the encrypted wallet stores scans of the user's benefits documents (IDs, pay stubs, award letters) **on-device only**, as `EncryptedFile` blobs behind a `BiometricPrompt` gate, excluded from cloud backup + device transfer, and **never** part of cloud sync. Nothing is transmitted → **Photos & videos** and **Files & docs** stay **No** (same on-device-only basis as the camera OCR). The new `USE_BIOMETRIC` permission gates local decryption only; it collects/shares no data → Section B and Section C are unchanged.
>
...
6. **#1514 (My Documents wallet):** adds the `USE_BIOMETRIC` permission and an on-device encrypted document store. Documents are `EncryptedFile`-encrypted at rest, biometric-gated, excluded from cloud backup + device transfer, and never synced — nothing is transmitted, so the mapping is unchanged: **Photos & videos / Files & docs = Not collected**, on the same on-device-only basis as the camera. `USE_BIOMETRIC` gates local decryption only (no data collected/shared) → Sections B and C unchanged. privacy.md §2.11 + §7 + the walkthrough §IV permission table document the posture; point a Play reviewer at the on-device-only + excluded-from-backup justification.
