...
- **Optional cloud sync (off by default):** when the user signs in, library/follows/positions/bookmarks/highlights/notes/pronunciation-dictionary/settings sync to the user's own **InstantDB** backend — **encrypted in transit**; any saved API keys are **end-to-end encrypted** behind a user-held passphrase. Cloud deletion is the in-app **Delete cloud data** action (#1248 — `purgeRemoteData` across all 8 syncer domains); **sign-out only revokes the token and wipes local data**, and the email / identity record is removed on email request. → Data Safety: *Data is encrypted in transit* = Yes; *Users can request deletion* = Yes.
- **Sharing → Yes, with InstantDB.** With sync on, email + user ID + app activity go to **InstantDB**, the sync **data processor** — declare *Data shared = Yes* on the Console form. No **other** third-party sharing: the remaining optional features call third parties **only at the user's direction with the user's own credentials** (Azure TTS, the BYO-key AI assistant, the chat/Notion sources).
